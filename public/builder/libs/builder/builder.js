...
function ensureVvvebId(el) {
    ...
    if (el.hasAttribute("data-vvveb-id"))
        return el.getAttribute("data-vvveb-id");
...
function resolveVvvebTargetById(id) {
    ...
    try {
        if (window.FrameDocument) {
            const el = window.FrameDocument.querySelector(
                `[data-vvveb-id="${id}"]`,
            );
            if (el) return el;
        }
    ...
    try {
        if (Vvveb && Vvveb.Builder && Vvveb.Builder.frameBody) {
            const doc =
                Vvveb.Builder.frameBody.ownerDocument ||
...
Vvveb.WysiwygEditor = {
    ...
    atageditorSetStyle: function (tag, style = {}, toggle = false) {
        try {
                ...
                if (selection.isCollapsed) {
                    ...
                    (selection.focusNode.nodeType === 3
                        ...
                        : selection.focusOffset ===
                          selection.focusNode.childNodes.length)
                ) {
                    ...
                    try {
                        ...
                        for (
                            let i = 0;
                            i < ancestorsFullySelected.length;
                            i++
                        ) {
            ...
            if (
                ...
                let textNode = iframeDoc.createTextNode(
                    element.textContent || "",
                );
                element.replaceWith(textNode);
                element = textNode;
    ...
    init: function (doc) {
        ...
        const textAlignButtondiv = document.getElementById(
        ...
        document
            ...
            .addEventListener("click", function (e) {
                ...
                setTimeout(function () {
                    ...
                    const linkInput =
                        document.getElementById("popup-link-input");
                    const linkTarget =
                        document.getElementById("popup-link-target");
                    ...
                    if (!selected || selected.tagName !== "A") {
                        if (sel && sel.rangeCount) {
                            ...
                            while (
                                anchor &&
                                anchor !==
                                    (frameWin ? frameWin.document : document) &&
                                anchor.tagName !== "A"
                            ) {
                    ...
                    if (linkTarget)
                        linkTarget.checked =
                            selected.getAttribute("target") === "_blank";
    ...
    edit: function (element) {
        ...
        function colorValueToHex(value) {
            ...
            try {
                ...
                if (p)
                    return `#${toHex(p.r)}${toHex(p.g)}${toHex(p.b)}`.toLowerCase();
            } catch (e) {
                /* ignore */
            }
        ...
        const backRaw = Vvveb.StyleManager.getStyle(
            element,
            "background-color",
        );
    ...
    destroy: function (element) {
        ...
        try {
            ...
            const newValue =
                node.innerHTML != null ? String(node.innerHTML) : "";
...
Vvveb.Builder = {
    ...
    _loadIframe: function (url) {
        ...
        return this.documentFrame.addEventListener("load", function () {
            ...
            let sectionEditBox = document.getElementById(
        ...
        if (window.FrameDocument && window.FrameDocument.body) {
            ...
            window.FrameDocument.body.addEventListener("click", function (e) {
                ...
                const realBtnElements = Array.from(container.children).filter(
                    (child) => {
                        if (child.hasAttribute("data-vvveb-helpers"))
                            return false;
                        if (child.classList.contains("vvveb-add-btn"))
                            return false;
                        if (child.classList.contains("vvveb-add-link-btn"))
                            return false;
                ...
                container.insertBefore(
                    window.FrameDocument.createTextNode(sep),
                    btn,
                );
                ...
                if (addSlideBtn) {
                    ...
                    if (slides.length > 0 && slides.length < 10) {
                        ...
                        if (swiperContainer.swiper) {
                            ...
                            const newSlideIndex =
                                swiperContainer.swiper.slides.length - 1;
                            ...
                            win.document
                                ...
                                .forEach((s) => {
                                    if (s.swiper) {
                                        s.swiper.update();
                                        if (s.swiper.pagination)
                                            s.swiper.pagination.render();
                                    }
                                });
                        }
    ...
    cloneNode: function (node) {
        ...
        function getNavButton(el) {
            ...
            return (
                el.querySelector?.('.nav-link,[data-bs-toggle="tab"]') || null
            );
        }
        ...
        function resolvePaneFromButton(btn, container) {
            ...
            const links = nav
                ? Array.from(
                      nav.querySelectorAll('.nav-link,[data-bs-toggle="tab"]'),
                  )
                : [];
        ...
        function normalizeTabsBlock(container) {
            ...
            if (!nav) return;
            nav.querySelectorAll('.nav-link,[data-bs-toggle="tab"]').forEach(
                (btn) => normalizeOneTabButton(btn, container),
            );
        }
        ...
        const isEl = node && node.nodeType === 1;
        const closest = (sel) =>
            isEl && node.closest ? node.closest(sel) : null;
        ...
        if (accItem) {
            ...
            target =
                tabItem.closest(".nav-item") || getNavButton(tabItem) || node;
            mode = "TAB_ITEM";
    ...
    selectNode: function (node) {
        ...
        if (node.tagName.toLowerCase() === "div") {
            ...
            if (downBtn)
                downBtn.style.setProperty("display", "none", "important");
        } else {
        ...
        try {
            ...
            if (iframe && iframe.contentDocument) {
                ...
                const activePencil = iframeDoc.querySelector(
                    "#section-edit-pencil",
                );
                if (activePencil) {
                    Vvveb.SectionEditor.destroy();
        ...
        if (self.texteditEl && self.selectedEl != node) {
            ...
            if (
                document.getElementById("wysiwyg-editor").style.display ===
                "none"
            ) {
    ...
    _initHighlight: function () {
        ...
        `,
                    ...
                    if (hoveredSection) {
                        const hoveredSectionRect =
                            hoveredSection.getBoundingClientRect();
                        document
                            .getElementById("section-edit-options")
                            .setAttribute(
                                "style",
                                `top:${
                                    hoveredSectionRect.top -
                                    (self.frameDoc.scrollTop ?? 0)
                                }px; 
                    ...
                    if (currentCard) {
                        ...
                        document
                            .getElementById("hovering-options")
                            .setAttribute(
                                "style",
                    ...
                    document.querySelector(
                        "#highlight-name .name",
                    ).textContent = getSectionIdForHIghlight(
                        event.target,
                        readableName,
                    );
                    if (!self.isDragging && !self.isResize) {
        ...
        let highlightDbClick = function (event) {
            if (Vvveb.Builder.isPreview == false) {
                if (!Vvveb.WysiwygEditor.isActive) {
                    ...
                    self.texteditEl.addEventListener(
                        "change",
                        _updateSelectBox,
                    );
                    _updateSelectBox();
    ...
    _initBox: function () {
        ...
        const InsertPanel = (() => {
            ...
            function tagBlockTiles() {
                ...
                tiles.forEach((li) => {
                    ...
                    const title =
                        li.querySelector(".name, .header, a, span")
                            ?.textContent || "";
        ...
        function applyBlockFilters() {
            ...
            tiles.forEach((li) => {
                ...
                li.style.display =
                    matchTxt && matchCatFromQuery && matchActiveCat
                        ? ""
                        : "none";
            });
        }
    ...
    cleanupAddLinkHelpersDom: function (doc) {
        try {
            ...
            doc.querySelectorAll("li.vvveb-add-link-helper").forEach((li) =>
                li.remove(),
            );
            ...
            doc.querySelectorAll(".add-card-btn[data-vvveb-helpers]").forEach(
                (btn) => btn.remove(),
            );
            doc.querySelectorAll(".vvveb-add-slide-btn").forEach((btn) =>
                btn.remove(),
            );
        } catch (e) {
...
function isLinkList(ul) {
    ...
    if (
        ul.closest("footer, header, nav, .footer, .site-footer, .footer-links")
    ) {
        return true;
    }
...
function addClonableCardHelpers(frameDoc) {
    ...
    cardContainers.forEach((container) => {
        ...
        if (container.querySelector(".add-card-btn[data-vvveb-helpers]"))
            return;
    ...
    iconGroups.forEach((container) => {
        ...
        if (
            container.querySelector(
                ".vvveb-add-link-btn[data-vvveb-context='icons']",
            )
        ) {
            return;
        }
                        ...
                        for (const n of m.removedNodes) {
                            try {
                                if (
                                    n &&
                                    n.getAttribute &&
                                    n.getAttribute("data-vvveb-helpers") ===
                                        "true"
                                ) {
                                    delay(() => {
                                        try {
...
function addButtonHelpers(frameDoc) {
    ...
    frameDoc.querySelectorAll("[data-btn]").forEach((el) => {
        ...
        const directBtnElements = Array.from(parent.children).filter(
            (child) => {
    ...
    newPage: function () {
        ...
        let submitForm = function (e) {
            ...
            this.querySelectorAll(
                ...
                if (el.offsetParent || el.name == "image")
            ...
            if (detail.page) {
                fetch(deleteUrl, {
                    ...
                    .catch((error) => {
                        // console.log(error);
                        let message =
                            error.statusText ?? "Error deleting page!";
                        displayToast("bg-danger", "Error", message);
        ...
        if (selected && selected.tagName === "IMG") {
            ...
            Vvveb.MediaModal.open(null, function (imageData) {
                ...
                if (selected && selected.tagName === "IMG" && payload.src) {
                    ...
                    const hadDesc = selected.hasAttribute(
                        "data-media-description",
                    );
                    const oldDesc = selected.getAttribute(
                        "data-media-description",
                    );
                    selected.setAttribute("src", payload.src);
...
function openContainerPopup(containerNode) {
    ...
    // Set Type
    let type = classes.find(
        (c) => c === "container" || c === "container-fluid",
    );
...
Vvveb.SectionEditor = {
    ...
    edit: function (element) {
        if (!["section", "footer"].includes(element.tagName.toLowerCase()))
            return;
    ...
    applyGradient: function (opts) {
        ...
        if (!recordUndo) {
            if (this._gradientPreviewOriginalStyle == null) {
                this._gradientPreviewOriginalStyle =
        ...
        var c1 =
            (document.getElementById("section-grad-c1") || {}).value ||
            "#000000";
        var a1 =
            parseInt(
                (document.getElementById("section-grad-c1a") || {}).value ||
                    "60",
                10,
            ) / 100;
        var c2 =
            (document.getElementById("section-grad-c2") || {}).value ||
            "#000000";
        var a2 =
            parseInt(
                (document.getElementById("section-grad-c2a") || {}).value ||
                    "0",
                10,
            ) / 100;
        var angle = parseInt(
            (document.getElementById("section-grad-angle") || {}).value ||
                "180",
            10,
        );
    ...
    destroy: function () {
        ...
        const sectionEditOptions = document.getElementById(
            "section-edit-options",
        );
...
Vvveb.FormEditor = {
    ...
    buildFields(form) {
        ...
        form.querySelectorAll("div[form-question-zigrow]").forEach(
            (wrapper, index) => {
...
Vvveb.LinkEditor = {
    ...
    `;
        ...
        this.modal
            .querySelector("#link-cancel")
            .addEventListener("click", close);
        this.modal
            .querySelector("#link-close-x")
            .addEventListener("click", close);
        ...
        this.modal.querySelectorAll(".vvv-link-style-reset").forEach((btn) => {
            btn.addEventListener("click", (e) => {
                ...
                if (key === "size") {
                    const toggle =
                        this.modal.querySelector("#link-size-toggle");
                    if (toggle) {
                        toggle
                            .querySelectorAll(".vvv-size-option")
                            .forEach((b) => b.classList.remove("is-active"));
                ...
                if (key === "radius") {
                    ...
                    if (wrap) {
                        wrap.querySelectorAll(".vvv-radius-option").forEach(
                    ...
                    // hidden values clear
                    const radiusHidden =
                        this.modal.querySelector("#link-style-radius");
                    const borderHidden =
                        this.modal.querySelector("#link-style-border");
    ...
    open(el) {
        ...
        const isButton =
            this.el.tagName === "A" && this.el.hasAttribute("data-btn");
  ...
  `,
        ...
        `;
            ...
            const ccSelect = this.fieldsWrap.querySelector(
                "#link-phone-country",
            );
            ...
            if (href) {
                try {
                    ...
                    if (msgInput)
                        msgInput.value = decodeURIComponent(
                            text.replace(/\+/g, ""),
                        );
                } catch (error) {
                    // ignore parse errors
            ...
            const pickBtn = this.fieldsWrap.querySelector(
    ...
    _sectionsOptions() {
        ...
        const doc =
            window.FrameDocument || Vvveb?.Builder?.iframe?.contentDocument;
        if (!doc) return "";
...
Vvveb.GlobalCustomVariable = {
    ...
    _findLogoTargets(doc) {
        ...
        doc.querySelectorAll(
            "[data-logo='navbar'],[data-logo='footer']",
    ...
    _mutateLogo(container, mode, value, size = 48) {
        ...
        if (mode === "text") {
            ...
            if (!span) {
                const existing =
                    cleanText || (container.textContent || "").trim();
                removeAllExcept([]);
    ...
    applyLogo({ mode, value, size }) {
        ...
        const getContainer = (el) =>
            el.querySelector("h1,h2,h3,h4,h5,h6") || el;
...
(function () {
    function injectIconCss(href, id = "fa-cdn") {
        const doc =
            Vvveb?.Builder?.iframe?.contentDocument || window.FrameDocument;
        if (!doc?.head) return;
        let link =
...
Vvveb.IconCustomLibrary = {
        ...
        this.search?.addEventListener("input", (e) => {
            ...
            if (this._faList) {
                const count = (style) =>
                    this._faList.filter(
                        (it) =>
                            it.style === style && (!q || it.search.includes(q)),
                    ).length;
                ...
                if (
                    ...
                    if (this.activeStyle === "solid" && cSolid === 0) {
                        if (cBrands > 0) this._activateStyleTab("brands");
                            ...
                            this._activateStyleTab("regular");
    ...
    async open(el) {
        ...
        this.modal
            ...
            .forEach((b) =>
                b.classList.toggle(
                    "active",
                    b.dataset.style === this.activeStyle,
                ),
            );
  ...
  `,
    ...
    _updateHeader(total) {
        const styleLabel =
            this.activeStyle.charAt(0).toUpperCase() +
            this.activeStyle.slice(1);
        if (this._packChip) {
...
Vvveb.IconSettings = {
    ...
    init() {
        ...
        this.colorSwatch = document.getElementById(
            "icon-settings-color-swatch",
        );
...
const CanvasInteractions = {
    ...
    _showTip(el, text) {
        ...
        if (top + tipHeight + 8 > viewportBottom) {
            const aboveTop =
                iframeRect.top + r.top - tipHeight - config.tipOffsetY;
...
window.addEventListener("vvveb.iframe.loaded", function () {
    ...
    CanvasInteractions.register({
        ...
        onDblClick(el, e) {
            ...
            const mode =
                clickedImg && el.contains(clickedImg) ? "image" : "text";
                ...
                Vvveb.MediaModal.open(null, function (imgData) {
                    const payload =
                        typeof imgData === "string"
                            ? { src: imgData }
                            : imgData || {};
                    if (!imgEl || imgEl.tagName !== "IMG" || !payload.src)
                        return;

                    const hadTitle = imgEl.hasAttribute("title");
                    const oldTitle = imgEl.getAttribute("title");
                    const hadDesc = imgEl.hasAttribute(
                        "data-media-description",
                    );
                    const oldDesc = imgEl.getAttribute(
                        "data-media-description",
                    );
...
(function () {
    ...
    function initToggle() {
        const wrap = document.querySelector(
            "#insert-modal .insert-grid-toggle",
        );
        if (!wrap) return;
